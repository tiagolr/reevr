...
void View::drawPoints(Graphics& g)
{
    ...
    if (!selectedPoint && !selectedMidpoint && hoverPoint) { // draw hovered point
    ...
    if (selectedPoint) { // draw selected point
...
void View::mouseUp(const juce::MouseEvent& e)
{
    ...
    else if (!e.mods.isRightButtonDown()) 
    {
        ...
        else if (preSelectionStart.x > -1 && (std::abs(preSelectionStart.x - preSelectionEnd.x) > 4 || // finished creating selection
            std::abs(preSelectionStart.y - preSelectionEnd.y) > 4)) 
        { 
            multiSelect.makeSelection(e, preSelectionStart, preSelectionEnd);
        }
        else if (multiSelect.mouseHover > -1) { // finished dragging selection
            multiSelect.mouseUp(e); // FIX - points may have been inverted due to selection drag
        } 
        else if (!multiSelect.selectionPoints.empty()) {
            multiSelect.clearSelection();
        }
